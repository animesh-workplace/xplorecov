...
<<<<<<< HEAD
from ..models import VirusStrain, Location, AAMutation, NucMutation, StructuralVariation, QualityMetrics, parse_aamutation_string
=======
from ..models import (
    VirusStrain,
    Location,
...
<<<<<<< HEAD
                ...
                collection_date=pandas.to_datetime(row['date']).date() if pandas.notna(row['date']) else None,
                host=row.get('host'),
                age=row.get('age') if pandas.notna(row.get('age')) else None,
                sex=row.get('sex'),
                coverage=float(row['coverage']) if pandas.notna(row.get('coverage')) else None,
...
<<<<<<< HEAD
            ...
            if pandas.notna(row.get('aaSubstitutions')):
                aa_mutation_objects.extend(_process_aa_mutations(strain, row['aaSubstitutions']))

            # Process nucleotide mutations
            if pandas.notna(row.get('substitutions')):
                nuc_mutation_objects.extend(_process_nuc_mutations(strain, row['substitutions']))

            # Create StructuralVariation object linked to strain
            structural_variation_objects.append(StructuralVariation(
                strain=strain,
                total_insertions=int(row['totalInsertions']) if pandas.notna(row.get('totalInsertions')) else 0,
                insertion_positions=row.get('insertions'),
                total_frameshifts=int(row['totalFrameShifts']) if pandas.notna(row.get('totalFrameShifts')) else 0,
                frameshift_positions=row.get('frameShifts'),
                total_missing=int(row['totalMissing']) if pandas.notna(row.get('totalMissing')) else 0,
                missing_ranges=row.get('missing')
            ))

            # Create QualityMetrics object linked to strain
            quality_metrics_objects.append(QualityMetrics(
                strain=strain,
                total_non_acgtns=int(row['totalNonACGTNs']) if pandas.notna(row.get('totalNonACGTNs')) else 0,
                non_acgtn_positions=row.get('nonACGTNs'),
                partially_aliased=bool(row['partiallyAliased']) if pandas.notna(row.get('partiallyAliased')) else False,
                scorpio_call=row.get('scorpio_call'),
                scorpio_support=float(row['scorpio_support']) if pandas.notna(row.get('scorpio_support')) else None,
                scorpio_conflict=float(row['scorpio_conflict']) if pandas.notna(row.get('scorpio_conflict')) else None
            ))
=======
...
<<<<<<< HEAD
    VirusStrain.objects.bulk_create(virus_strain_objects, batch_size=len(virus_strain_objects))
    Location.objects.bulk_create(location_objects, batch_size=len(location_objects))
    AAMutation.objects.bulk_create(aa_mutation_objects, batch_size=len(aa_mutation_objects))
    NucMutation.objects.bulk_create(nuc_mutation_objects, batch_size=len(nuc_mutation_objects))
    StructuralVariation.objects.bulk_create(structural_variation_objects, batch_size=len(structural_variation_objects))
    QualityMetrics.objects.bulk_create(quality_metrics_objects, batch_size=len(quality_metrics_objects))
...
def _process_aa_mutations(strain, mutations_str):
    ...
    for mutation in mutations:
        ...
        aa_mutation_objects.append(AAMutation(
            strain=strain,
...
<<<<<<< HEAD
        nuc_mutation_objects.append(NucMutation(
            strain=strain,
            position=position,
